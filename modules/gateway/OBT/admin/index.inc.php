...
if(isset($_GET['reset']) && $_GET['reset']){
	$db->misc("DELETE FROM `".$glob['dbprefix']."CubeCart_Modules` WHERE `module` = 'gateway' AND `folder` = 'OBT'");
	$db->misc("DELETE FROM `".$glob['dbprefix']."CubeCart_config` WHERE `name` = 'OBT'");
...
if(isset($_POST['module']['emailVerified']) && $_POST['module']['emailVerified']==false){
	...
	if($respParts[0] == "OK") {
		...
		$_POST['module']['cust_id'] = $respParts[1];
	} else {
		//$accountEmail = false;
...
if(!$_POST['module']['quickbefore'] && $_POST['module']['quick'] && $module['cust_id']>0){
	...
	$body = "Platform Name: CubeCart ".$ini['ver']."\n";
...
?>
<p><a href="http://www.nextgenerationpayments.com/en/home/" target="_blank"><img src="modules/<?php echo $moduleType; ?>/<?php echo $moduleName; ?>/admin/logo.gif" alt="" border="0" title="" /></a></p>
<p class="copyText">&quot;Hassle-free payments with Online Bank Transfer.&quot;</p>
<p>To have access to the Online Bank Transfer payment network of Moneybookers please <a href='https://www.moneybookers.com/app/register.pl' class="txtLink">register here</a> for a free account if you don’t have one yet.</p> More information can be found <a href='http://www.moneybookers.com/partners/cubecart/' class='txtLink'>here</a>.
<?php
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" class="mainTable">
  ...
  <tr>
    <td width="30%" align="left" class="tdText"><strong>Use iFrame?</strong><br />This prevents the customer being taken away from the store for payment.</td>
    <td class="tdText">
	<select name="module[iframe]">
		<option value="1" <?php if(!isset($module['iframe']) || $module['iframe']==1) echo "selected='selected'"; ?>>Yes</option>
		<option value="0" <?php if(isset($module['iframe']) && $module['iframe']==0) echo "selected='selected'"; ?>>No</option>
    </select>
	</td>
  </tr>
  ...
  <tr>
    ...
    <td class="tdText">
    <input type="hidden" name="module[logo]" value="1" />
    <input type="hidden" name="module[more_link]" value="http://www.nextgenerationpayments.com/en/customerinfo" />
    <input type="hidden" name="module[desc]" value="Online Bank Transfer" />
