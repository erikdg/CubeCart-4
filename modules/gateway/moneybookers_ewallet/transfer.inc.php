...
function fixedVars() {
	...
	$hiddenVars = "<input type='hidden' name='pay_to_email' value='".$module['email']."' />
		...
		<input type='hidden' name='return_url' value='".$GLOBALS['storeURL']."/index.php?_g=rm&amp;type=gateway&amp;cmd=process&amp;module=moneybookers&amp;cart_order_id=".$orderSum['cart_order_id']."' />
		<input type='hidden' name='cancel_url' value='".$GLOBALS['storeURL']."/index.php?_g=rm&amp;type=gateway&amp;cmd=process&amp;module=moneybookers&amp;cart_order_id=".$orderSum['cart_order_id']."&amp;cancelled=true' />
...
$formMethod = 'post';
$formTarget = '_self';
$transfer	= $module['iframe'] ? 'manual' : 'auto';
$stateUpdate= true;
