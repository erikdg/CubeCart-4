...
function func_https_request($method, $url, $data="", $join="&", $cookie="", $conttype="application/x-www-form-urlencoded", $referer="", $cert="", $kcert="", $headers="", $timeout = 0)
{
	if( !function_exists('curl_init') )
		return array("0","CubeCart HTTPS: libcurl is not supported");

	if(($method!="POST") && ($method!="GET"))
		return array("0","CubeCart HTTPS: Invalid method");

	if(!preg_match("/^(https?:\/\/)(.*\@)?([a-z0-9_\.\-]+):(\d+)(\/.*)$/Ui",$url,$m))
		return array("0","CubeCart HTTPS: Invalid URL");
	...
	curl_close ($ch); 
	if( $error )
		return array("0","CubeCart HTTPS: libcurl error($errno): $error");
