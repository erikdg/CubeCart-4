			...
			if(is_array($taxTypes))
			{
				...
				foreach($taxTypes as $key => $value) {
					...
					if($config_tax_mod['status']==1) {
						...
						$query = "SELECT t.tax_percent, c.iso, county_id, shipping FROM ".$glob['dbprefix']."CubeCart_tax_rates t LEFT JOIN ".$glob['dbprefix']."CubeCart_iso_countries c ON t.country_id = c.id WHERE `type_id` = 1 AND `active` = 1 ORDER BY tax_percent DESC";
